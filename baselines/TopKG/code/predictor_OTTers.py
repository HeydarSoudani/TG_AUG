...
# login(token="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", add_to_git_credential=True)
...
class KeywordPredictor(pl.LightningModule):
    def __init__(self, batch_size=None, plan_type=None, lr=None, num_workers=None, **kwargs):
        ...
        self.tok: BertTokenizer = BertTokenizer.from_pretrained('bert-base-uncased')
        ...
        self.pr_positive = MetricCollection([
            ...
            Precision(task="multiclass", num_classes=2, ignore_index=0),
            Recall(task="multiclass", num_classes=2, ignore_index=0),
            F1Score(task="multiclass", num_classes=2, ignore_index=0),
        ])
        self.pr_negative = MetricCollection([
            # Precision(ignore_index=1),
            # Recall(ignore_index=1),
            # F1Score(ignore_index=1),
            Precision(task="multiclass", num_classes=2, ignore_index=1),
            Recall(task="multiclass", num_classes=2, ignore_index=1),
            F1Score(task="multiclass", num_classes=2, ignore_index=1),
        ])
        self.bleu = BLEU()
